...
**阿山** 是连接强大 AI 智能体与普通用户之间的桥梁。基于 [OpenClaw](https://github.com/OpenClaw) 构建，它将命令行式的 AI 编排转变为易用、美观的桌面体验，无需使用终端。

无论是自动化工作流、连接通讯软件，还是调度智能定时任务，阿山都能提供高效易用的图形界面，帮助你充分发挥 AI 智能体的能力。

阿山预置了最佳实践的模型供应商配置，原生支持 Windows 平台以及多语言设置。当然，你也可以通过 **设置 → 高级 → 开发者模式** 来进行精细的高级配置。
...
构建 AI 智能体不应该需要精通命令行。阿山的设计理念很简单：**强大的技术值得拥有一个尊重用户时间的界面。**
...
阿山直接基于官方 **OpenClaw** 核心构建。无需单独安装，我们将运行时嵌入应用内部，提供开箱即用的无缝体验。
...
### 🗓️ 本地日程表
内置类似 mac 日历风格的本地日程页，支持月视图、当天安排、全天事件、颜色标记与快速搜索。当前版本默认把日程保存在本地应用存储中，不依赖系统日历权限。
...
如果你在中国大陆或当前网络无法稳定访问 GitHub，`pnpm run package:mac:local` 可能会卡在下载内置 `uv`。这种情况下可以任选一种方式：

```bash
# 方式 1：通过镜像基础地址下载 uv（示例变量名）
UV_DOWNLOAD_BASE_URL=<你的 uv 镜像基础地址> pnpm run package:mac:local

# 方式 2：先手动把 uv 放到当前架构目录，再打包
# Apple Silicon Mac:
