...
# Post-installation script for 阿山 on Linux
...
# Create symbolic link for 阿山 app binary
if [ -x /opt/阿山/shan ]; then
    ln -sf /opt/阿山/shan /usr/local/bin/shan 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/阿山/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/阿山/chrome-sandbox' || true
else
    chmod 0755 '/opt/阿山/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/阿山/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/shan'
...
echo "阿山 has been installed successfully."
