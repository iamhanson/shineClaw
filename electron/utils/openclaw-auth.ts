...
export async function getActiveOpenClawProviders(): Promise<Set<string>> {
  ...
  try {
    ...
    const modelsProviders = providers && typeof providers === 'object'
      ? (providers as Record<string, unknown>)
      : {};
    if (providers && typeof providers === 'object') {
      for (const key of Object.keys(modelsProviders)) {
    ...
    if (primaryModel?.includes('/')) {
      const providerKey = primaryModel.split('/')[0];
      const isMultiInstanceProviderKey =
        providerKey === 'custom'
        || providerKey === 'ollama'
        || providerKey.startsWith('custom-')
        || providerKey.startsWith('ollama-');
      const existsInModelsProviders = Object.prototype.hasOwnProperty.call(modelsProviders, providerKey);

      // Guard against stale model prefixes such as custom-<uuid> that can linger
      // in agents.defaults.model.primary after provider key migration. For
      // multi-instance providers, only treat the prefix as active when it is an
      // explicit models.providers key.
      if (!isMultiInstanceProviderKey || existsInModelsProviders) {
