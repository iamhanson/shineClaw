...
export async function handleProviderRoutes(
  ...
  if (url.pathname === '/api/provider-accounts/default' && req.method === 'PUT') {
    try {
      ...
      logger.info(`[route] PUT /api/provider-accounts/default accountId="${body.accountId}"`);
      console.log(`[route] PUT /api/provider-accounts/default accountId="${body.accountId}"`);
      ...
      logger.info(`[route] PUT /api/provider-accounts/default DONE`);
  ...
  if (url.pathname.startsWith('/api/provider-accounts/') && req.method === 'PUT') {
    ...
    try {
      ...
      const hasPatchChanges = hasObjectChanges(
